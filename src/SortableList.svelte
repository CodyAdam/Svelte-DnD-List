...
const reorder = ({from, to}) => {
  ...
  while (from < 0) from += newCards.length;
  while (to < 0) to += newCards.length;
  if (to >= newCards.length) {
    let k = to - newCards.length + 1;
    while (k--) newCards.push(undefined);
  }
  newCards.splice(to, 0, newCards.splice(from, 1)[0]);

  dispatch("sort", newCards);
};
// UTILS
